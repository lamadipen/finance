...
class DatumLegendWithMeasures extends StatelessWidget {
  ...
  static List<charts.Series<LinearSales, int>> _createSampleData(transactions) {
    List<LinearSales> data = List<LinearSales>.from(transactions
        .map((Transaction t) => LinearSales(t.date.year, t.amount.toInt())));
    // final data = [
  ...
  /// Create series list with one series
  static List<charts.Series<LinearSales, int>> _createSampleDataBackup() {
    final data = [
      new LinearSales(0, 100),
      new LinearSales(1, 75),
      new LinearSales(2, 25),
